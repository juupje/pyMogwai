...
from mogwai.io import GraphSON, GraphML, RDF, IOBackend
import logging
...
class IO(Step):
    def __init__(self, traversal:'Traversal', file_path:str, read:bool=None, write:bool=None, **kwargs):
    ...
    def with_(self, value):
        if type(value) is tuple and len(value) == 2:
            if value[0] == IOEnum.reader:
                self.read = True
            elif value[0] == IOEnum.writer:
                self.write = True
            self.backend = value[1]
        else:
            raise QueryError("with() expected arguments (IO.reader, backend) or (IO.writer, backend)")
    ...
    def build(self):
        ...
        if self.backend is None:
            ...
            if self.backend not in [IOEnum.graphson, IOEnum.graphml, IOEnum.rdf]:
    ...
    def __call__(self, traversers: Iterable['Traverser']) -> Iterable['Traverser']:
        ...
        if self.task == IOEnum.reader:
            #read from file
            graph = backend.read(self.file_path)
